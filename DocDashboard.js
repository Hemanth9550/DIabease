...
const DocDashboard = ({ route }) => {
  ...
  useEffect(() => {
    navigation.setOptions({
      ...
      headerTitle: null,
      headerLeft: () => (
        <TouchableOpacity style={styles.menuButton} onPress={() => navigation.navigate('Docmenu', { doctorId,doctorname,email })}><FontAwesome name="bars" size={24} color="white" />
        </TouchableOpacity>
      ),
  ...
  const fetchRecentPatients = () => {
    axios
      .get(`http://192.168.206.1/demo/fetch_data.php?doctorId=${doctorId}`)
      .then((response) => {
