...
const PatientLogin = () => {
  ...
  const handleLogin = async () => {
    try {
      ...
      const response = await fetch('http://192.168.206.1/demo/Loginp.php', {
        method: 'POST',
        headers: {
      ...
      if (data.status === 'success') {
        ...
        // Navigate to PatDashboard and pass data
        navigation.navigate('PatDashboard', {
          id: data.id,
          name: data.name,
          mail: data.mail,
          P_Id: data.P_Id,
          doctorId: data.doctorId, // Pass doctorId to PatDashboard
          age: data.age, // Pass age
          mob: data.mob, // Pass mobile number
          occupation: data.occupation // Pass occupation
        });
      } else {
