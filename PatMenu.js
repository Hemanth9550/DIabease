...
const PatMenu = () => {
  ...
  const { name, mail, id, P_Id, age, mob, occupation, doctorId } = route.params || {};
  ...
  useEffect(() => {
    const fetchProfileImage = async () => {
      try {
        const response = await axios.get(`http://192.168.206.1/demo/image.php?P_Id=${P_Id}`);
  ...
  const navigateBack = () => {
    navigation.navigate('PatDashboard', { id, name, mail, P_Id });
  };

  const handleNavigateToBloodSugar = () => {
    navigation.navigate('BloodSugar', { id, name, mail, P_Id });
  };
  ...
  const handleEditProfile = () => {
    navigation.navigate('EditProfile', { id, name, mail, P_Id, age, mob, occupation, profileImage });
  };
