...
const EditProfile = ({ route, navigation }) => {
  ...
  const fetchPatientDetails = async () => {
    try {
      const response = await fetch(`http://192.168.206.1/demo/fetch_profile.php?P_Id=${P_Id}`);
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch('http://192.168.206.1/demo/update_profile.php', {
        method: 'POST',
        body: formData,
        headers: {
    ...
    <ScrollView contentContainerStyle={styles.scrollContainer}>
      <View style={styles.container}>
        <View style={styles.headerContainer}>
          ...
          <Text style={styles.headerTitle}>Edit Patient Profile</Text>
          <View style={styles.headerButtonPlaceholder} />
        </View>
